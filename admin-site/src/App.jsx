...
export default function App() {
  ...
  const pages = { dashboard: <Dashboard />, products: <Products />, categories: <Categories />, orders: <Orders />, users: <Users /> };
 
  return (
    <div style={{ ...theme, display: "flex", height: "100vh", width: "100vw", position: "fixed",top: 0, left: 0, fontFamily: "'DM Sans', system-ui, sans-serif",
      ...
      <aside style={{ width: 220, background: "var(--card)", borderRight: "1px solid var(--border)",
        ...
        <div style={{ padding: "14px 16px", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#6366f1",
            display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 13 }}>A</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text)" }}>Admin</div>
            <div style={{ fontSize: 10, color: "var(--muted)" }}>admin@store.com</div>
          </div>
        </div>
      </aside>
