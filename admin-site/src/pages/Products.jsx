...
export default function Products() {
  ...
  const addSizePrice = () =>
    setForm((f) => ({
      ...f,
      sizePrices: [...f.sizePrices, { label: "", price: "" }],
    }));

  const removeSizePrice = (i) =>
    setForm((f) => ({
      ...f,
      sizePrices: f.sizePrices.filter((_, idx) => idx !== i),
    }));
  ...
  const handleSave = async () => {
    ...
    if (!form.isContactPrice && !form.price)
      return alert("Vui lòng nhập giá bán");
        ...
        >
          <thead style={{ background: "var(--bg)" }}>
            ...
            >
              ...
              <th style={{ textAlign: "center", padding: "14px 16px" }}>
                Tồn kho
              </th>
              <th style={{ textAlign: "center", padding: "14px 16px" }}>
                Hiển thị
              ...
              items.map((p) => (
                <tr key={p.id} style={{ borderTop: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "12px 16px", textAlign: "center" }}>
                    <Badge
                      status={p.isOutOfStock ? "out_of_stock" : "active"}
                    />
                  </td>
                  {/* Hiển thị */}
                  <td style={{ padding: "12px 16px", textAlign: "center" }}>
                ...
                {form.sizePrices.map((sp, i) => (
                  ...
                  >
                    <input
                      ...
                      value={sp.label}
                      onChange={(e) =>
                        updateSizePrice(i, "label", e.target.value)
                      }
                      style={{
                        flex: 1,
                        height: 38,
                    ...
                    <input
                      ...
                      value={sp.price}
                      onChange={(e) =>
                        updateSizePrice(i, "price", e.target.value)
                      }
                      style={{
                        width: 130,
                        height: 38,
