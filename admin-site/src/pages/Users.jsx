...
// ── Cols (dùng array tránh whitespace text node trong colgroup) ───────────────
const COLS = [
  { width: "22%" }, // Người dùng
  { width: "15%" }, // Vai trò
  { width: "13%" }, // Đơn hàng
...
const Pagination = ({ page, totalPages, onChange }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center", padding: "20px 0 4px" }}>
      <button style={{ ...btn(false), opacity: page === 1 ? 0.4 : 1 }} disabled={page === 1} onClick={() => onChange(page - 1)}>←</button>
      {pages.map((p, i) =>
        p === "..." ? (
      ...
      <button style={{ ...btn(false), opacity: page === totalPages ? 0.4 : 1 }} disabled={page === totalPages} onClick={() => onChange(page + 1)}>→</button>
    </div>
  );
};
...
const Users = () => {
  ...
  const fetchUsers = useCallback(async (p = 1, q = "") => {
    ...
    setError(null);
    try {
      const payload = await userApi.getAll({ page: p, pageSize: PAGE_SIZE, search: q });
      if (payload?.items) {
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try {
      await userApi.updateStatus(id, next); // ✅ đúng tên + STATUS_MAP xử lý trong userApi
    } catch {
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "var(--muted)", margin: "4px 0 0", fontSize: 13 }}>{total} tài khoản</p>
        </div>
      ...
      <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden" }}>
        ...
        {!loading && !error && users.length === 0 && (
          <div style={{ padding: 40, textAlign: "center", color: "var(--muted)", fontSize: 14 }}>Không tìm thấy người dùng nào.</div>
        )}

        {!loading && !error && users.length > 0 && (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed" }}>
            {/* ✅ dùng array → không có whitespace text node */}
            ...
            <thead style={{ background: "var(--bg)" }}>
              <tr style={{ color: "var(--muted)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                ...
                <th style={{ textAlign: "left",   padding: "14px 20px" }}>Vai trò</th>
                <th style={{ textAlign: "center", padding: "14px 20px" }}>Đơn hàng</th>
                <th style={{ textAlign: "right",  padding: "14px 20px" }}>Tổng chi</th>
                <th style={{ textAlign: "center", padding: "14px 20px" }}>Trạng thái</th>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} style={{ borderTop: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "14px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.userName}</div>
                  ...
                  {/* Orders */}
                  <td style={{ padding: "14px 20px", textAlign: "center", color: "var(--text)", fontWeight: 600 }}>{u.orderCount}</td>

                  {/* Spent */}
                  <td style={{ padding: "14px 20px", textAlign: "right", fontWeight: 700, color: "var(--text)" }}>
